...
float4 main_ps(float4 position : SV_Position, float2 uv : Texcoord) : SV_TARGET {
    ...
    for (int x = -1; x <= 1; x++) {
        for (int y = -1; y <= 1; y++) {
            ...
            float3 neighbor_color = _PostSourceTexture.Sample(_point_clamp_sampler, neighbor_uv).rgb;
            ...
            float neighbor_depth = _DepthBuffer.Sample(_point_clamp_sampler, neighbor_uv).r;
#ifdef GLACIER_REVERSE_Z
    ...
    float2 velocity = VelocityTexture.Sample(_point_clamp_sampler, closest_uv).xy;
    ...
    float3 prev_color = SampleTextureCatmullRom(PrevColorTexture, _point_clamp_sampler, prev_uv, _ScreenParam.xy).rgb;
    ...
    curr_weight *= 1.0f / (1.0f + RGBToLuminance(ldr_curr_color));
    prev_weight *= 1.0f / (1.0f + RGBToLuminance(ldr_prev_color));
