...
GameObject& BezierMesh::CreatePlaneRoad(const BezierCurve& curve, float half_width, Material* material, bool flatten) {
    VertexCollection vertices;
    IndexCollection indices;

    auto points = curve.GetPoints();
    int vert_size = points.size() * 2;
    int numTris = 2 * (points.size() - 1) + (curve.IsClosed() ? 2 : 0);
...
GameObject& BezierMesh::CreateRoad(const BezierCurve& curve, float half_width, float thickness, Material* material, bool flatten) {
    VertexCollection vertices;
    IndexCollection indices;
...
GameObject& BezierMesh::CreateTube(const BezierCurve& curve, float radius, int resolutionU, int resolutionV, Material* material) {
    VertexCollection vertices;
    IndexCollection indices;
